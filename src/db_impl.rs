...
pub struct DB {
    ...
    pub cache: Shared<TableCache>,
    pub vset: Shared<VersionSet>,
    snaps: SnapshotList,
...
impl DB {
    ...
    pub fn new<P: AsRef<Path>>(name: P, mut opt: Options) -> DB {
...
// READ //
impl DB {
    pub fn get_internal(&mut self, seq: SequenceNumber, key: &[u8]) -> Result<Option<Vec<u8>>> {
...
impl DB {
    ...
    /// if it's the case.
    pub fn make_room_for_write(&mut self, force: bool) -> Result<()> {
    ...
    pub fn maybe_do_compaction(&mut self) -> Result<()> {
        if self.imm.is_some() {
    ...
    pub fn compact_memtable(&mut self) -> Result<()> {
...
impl CompactionState {
    pub fn new(c: Compaction, smallest: SequenceNumber) -> CompactionState {
        CompactionState {
            compaction: c,
    ...
    pub fn cleanup<P: AsRef<Path>>(&mut self, env: &Box<dyn Env>, name: P) {
...
pub fn log_file_name(db: &Path, num: FileNum) -> PathBuf {
...
pub fn open_info_log<E: Env + ?Sized, P: AsRef<Path>>(env: &E, db: P) -> Logger {
    let db = db.as_ref();
